      ...
      isQuadrupedNarrow(new QCheckBox)
{
    ...
    table->setItem(0, 0, new TableWidgetItem("raycastDistanceUp"));
    table->setItem(0, 1, new TableWidgetItem("hkReal"));
    table->setCellWidget(0, 2, raycastDistanceUp);
    table->setItem(1, 0, new TableWidgetItem("raycastDistanceDown"));
    table->setItem(1, 1, new TableWidgetItem("hkReal"));
    table->setCellWidget(1, 2, raycastDistanceDown);
    table->setItem(2, 0, new TableWidgetItem("originalGroundHeightMS"));
    table->setItem(2, 1, new TableWidgetItem("hkReal"));
    table->setCellWidget(2, 2, originalGroundHeightMS);
    table->setItem(3, 0, new TableWidgetItem("verticalOffset"));
    table->setItem(3, 1, new TableWidgetItem("hkReal"));
    table->setCellWidget(3, 2, verticalOffset);
    table->setItem(4, 0, new TableWidgetItem("collisionFilterInfo"));
    table->setItem(4, 1, new TableWidgetItem("hkInt32"));
    table->setCellWidget(4, 2, collisionFilterInfo);
    table->setItem(5, 0, new TableWidgetItem("forwardAlignFraction"));
    table->setItem(5, 1, new TableWidgetItem("hkReal"));
    table->setCellWidget(5, 2, forwardAlignFraction);
    table->setItem(6, 0, new TableWidgetItem("sidewaysAlignFraction"));
    table->setItem(6, 1, new TableWidgetItem("hkReal"));
    table->setCellWidget(6, 2, sidewaysAlignFraction);
    table->setItem(7, 0, new TableWidgetItem("sidewaysSampleWidth"));
    table->setItem(7, 1, new TableWidgetItem("hkReal"));
    table->setCellWidget(7, 2, sidewaysSampleWidth);
    table->setItem(8, 0, new TableWidgetItem("lockFeetWhenPlanted"));
    table->setItem(8, 1, new TableWidgetItem("hkBool"));
    table->setCellWidget(8, 2, lockFeetWhenPlanted);
    table->setItem(9, 0, new TableWidgetItem("useCharacterUpVector"));
    table->setItem(9, 1, new TableWidgetItem("hkBool"));
    table->setCellWidget(9, 2, useCharacterUpVector);
    table->setItem(10, 0, new TableWidgetItem("isQuadrupedNarrow"));
    table->setItem(10, 1, new TableWidgetItem("hkBool"));
    ...
    if (data && data->getSignature() == HKB_FOOT_IK_DRIVER_INFO){
        ...
        for (int i = 0, k; i < bsData->legs.size(); i++){
            ...
            if (k >= table->rowCount()){
                ...
                table->setItem(k, 0, new TableWidgetItem("Leg "+QString::number(i)));
                table->setItem(k, 1, new TableWidgetItem("hkbFootIkDriverInfoLeg"));
...
void FootIkDriverInfoUI::addLeg(){
    if (bsData){
        ...
        if (result >= table->rowCount()){
            result--;
            table->setRowCount(table->rowCount() + 1);
            table->setItem(result, 0, new TableWidgetItem("Leg "+QString::number(bsData->legs.size() - 1)));
            table->setItem(result, 1, new TableWidgetItem("hkbFootIkDriverInfoLeg"));
