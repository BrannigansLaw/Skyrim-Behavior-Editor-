...
void EvaluateHandleModifierUI::connectToTables(GenericTableWidget *variables, GenericTableWidget *properties){
    if (variables && properties){
        ...
        CRITICAL_ERROR_MESSAGE("EvaluateHandleModifierUI::connectToTables(): One or more arguments are nullptr!!");
    }
}
...
void EvaluateHandleModifierUI::loadData(HkxObject *data){
    ...
    if (data){
        if (data->getSignature() == HKB_EVALUATE_HANDLE_MODIFIER){
            ...
            CRITICAL_ERROR_MESSAGE("EvaluateHandleModifierUI::loadData(): The data is an incorrect type!!");
        }
    }else{
        CRITICAL_ERROR_MESSAGE("EvaluateHandleModifierUI::loadData(): The data is nullptr!!");
    }
    connectSignals();
}
...
void EvaluateHandleModifierUI::setName(){
    if (bsData){
        ...
        CRITICAL_ERROR_MESSAGE("EvaluateHandleModifierUI::setName(): The data is nullptr!!");
    }
}
...
void EvaluateHandleModifierUI::viewSelected(int row, int column){
    if (bsData){
        ...
        CRITICAL_ERROR_MESSAGE("EvaluateHandleModifierUI::viewSelected(): The 'bsData' pointer is nullptr!!");
    }
}
...
void EvaluateHandleModifierUI::selectTableToView(bool viewisProperty, const QString & path){
    if (bsData){
        ...
        CRITICAL_ERROR_MESSAGE("EvaluateHandleModifierUI::selectTableToView(): The data is nullptr!!");
    }
}
...
void EvaluateHandleModifierUI::variableRenamed(const QString & name, int index){
    if (bsData){
        ...
        CRITICAL_ERROR_MESSAGE("EvaluateHandleModifierUI::variableRenamed(): The 'bsData' pointer is nullptr!!");
    }
}
...
bool EvaluateHandleModifierUI::setBinding(int index, int row, const QString &variableName, const QString &path, hkVariableType type, bool isProperty){
    ...
    if (bsData){
        if (index == 0){
            ...
            if (isProperty){
                if (!varBind->addBinding(path, index - 1, hkbVariableBindingSet::hkBinding::BINDING_TYPE_CHARACTER_PROPERTY)){
                    CRITICAL_ERROR_MESSAGE("EvaluateHandleModifierUI::setBinding(): The attempt to add a binding to this object's hkbVariableBindingSet failed!!");
                }
            }else{
        ...
        CRITICAL_ERROR_MESSAGE("EvaluateHandleModifierUI::setBinding(): The data is nullptr!!");
    }
    return true;
}
...
void EvaluateHandleModifierUI::loadBinding(int row, int column, hkbVariableBindingSet *varBind, const QString &path){
    if (bsData){
        ...
        CRITICAL_ERROR_MESSAGE("EvaluateHandleModifierUI::loadBinding(): The data is nullptr!!");
    }
}
