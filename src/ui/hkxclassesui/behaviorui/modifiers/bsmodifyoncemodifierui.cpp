...
void BSModifyOnceModifierUI::connectToTables(GenericTableWidget *modifiers, GenericTableWidget *variables, GenericTableWidget *properties){
    if (variables && properties && modifiers){
        ...
        CRITICAL_ERROR_MESSAGE("BSModifyOnceModifierUI::connectToTables(): One or more arguments are nullptr!!");
    }
}
...
void BSModifyOnceModifierUI::loadData(HkxObject *data){
    ...
    if (data){
        ...
        CRITICAL_ERROR_MESSAGE("BSModifyOnceModifierUI::loadData(): The data passed to the UI is nullptr!!!");
    }
    connectSignals();
}
...
void BSModifyOnceModifierUI::setName(){
    if (bsData){
        ...
        CRITICAL_ERROR_MESSAGE("BSModifyOnceModifierUI::setName(): The data is nullptr!!");
    }
}
        ...
        if (behaviorView){
            ...
            if (ptr){
                if (name != ptr->getName()){
                    ...
                    return;
                }else if (ptr == bsData || (row == ON_ACTIVATE_MODIFIER_ROW && !behaviorView->reconnectIcon(behaviorView->getSelectedItem(), mod, 0, ptr,  false)) || (row == ON_DEACTIVATE_MODIFIER_ROW && !behaviorView->reconnectIcon(behaviorView->getSelectedItem(), mod, 1, ptr, false))){
                ...
                if (behaviorView->getSelectedItem()){
                    ...
                    CRITICAL_ERROR_MESSAGE("BSModifyOnceModifierUI::setGenerator(): The selected icon is nullptr!!");
                    return;
                }
            }
            ...
            CRITICAL_ERROR_MESSAGE("BSModifyOnceModifierUI::setGenerator(): The 'behaviorView' pointer is nullptr!!");
        }
    }else{
        CRITICAL_ERROR_MESSAGE("BSModifyOnceModifierUI::setGenerator(): The 'bsData' pointer is nullptr!!");
    }
}
...
void BSModifyOnceModifierUI::selectTableToView(bool viewproperties, const QString & path){
    if (bsData){
        ...
        CRITICAL_ERROR_MESSAGE("BSModifyOnceModifierUI::selectTableToView(): The data is nullptr!!");
    }
}
...
void BSModifyOnceModifierUI::setBindingVariable(int index, const QString & name){
    if (bsData){
        ...
        CRITICAL_ERROR_MESSAGE("BSModifyOnceModifierUI::setBindingVariable(): The data is nullptr!!");
    }
}
...
bool BSModifyOnceModifierUI::setBinding(int index, int row, const QString & variableName, const QString & path, hkVariableType type, bool isProperty){
    ...
    if (bsData){
        if (index == 0){
            ...
            if (isProperty){
                if (!varBind->addBinding(path, index - 1, hkbVariableBindingSet::hkBinding::BINDING_TYPE_CHARACTER_PROPERTY)){
                    CRITICAL_ERROR_MESSAGE("BSModifyOnceModifierUI::setBinding(): The attempt to add a binding to this object's hkbVariableBindingSet failed!!");
                }
            }else{
        ...
        CRITICAL_ERROR_MESSAGE("BSModifyOnceModifierUI::setBinding(): The data is nullptr!!");
    }
    return true;
}

void BSModifyOnceModifierUI::loadBinding(int row, int column, hkbVariableBindingSet *varBind, const QString &path){
    if (bsData){
        if (varBind){
            ...
            CRITICAL_ERROR_MESSAGE("BSModifyOnceModifierUI::loadBinding(): The variable binding set is nullptr!!");
        }
    }else{
        CRITICAL_ERROR_MESSAGE("BSModifyOnceModifierUI::loadBinding(): The data is nullptr!!");
    }
}
    ...
    if (bsData){
        if (column == VALUE_COLUMN){
            if (row == ON_ACTIVATE_MODIFIER_ROW){
                emit viewModifiers(static_cast<BehaviorFile *>(bsData->getParentFile())->getIndexOfModifier(bsData->pOnActivateModifier) + 1, QString(), QStringList());
            }else if (row == ON_DEACTIVATE_MODIFIER_ROW){
                emit viewModifiers(static_cast<BehaviorFile *>(bsData->getParentFile())->getIndexOfModifier(bsData->pOnDeactivateModifier) + 1, QString(), QStringList());
            }
        }
    }else{
        CRITICAL_ERROR_MESSAGE("BSModifyOnceModifierUI::viewSelected(): The 'bsData' pointer is nullptr!!");
    }
}
...
void BSModifyOnceModifierUI::variableRenamed(const QString & name, int index){
    if (bsData){
        ...
        CRITICAL_ERROR_MESSAGE("BSModifyOnceModifierUI::variableRenamed(): The 'bsData' pointer is nullptr!!");
    }
}
...
void BSModifyOnceModifierUI::modifierRenamed(const QString & name, int index){
    if (bsData){
        ...
        CRITICAL_ERROR_MESSAGE("BSModifyOnceModifierUI::modifierRenamed(): The 'bsData' pointer is nullptr!!");
    }
}
