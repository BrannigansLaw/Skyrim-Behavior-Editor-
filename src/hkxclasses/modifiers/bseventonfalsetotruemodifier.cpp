...
BSEventOnFalseToTrueModifier::BSEventOnFalseToTrueModifier(BehaviorFile *parent, long ref)
...
bool BSEventOnFalseToTrueModifier::link(){
    ...
    HkxObjectExpSharedPtr *ptr = getParentFile()->findHkxObject(eventToSend1.payload.getReference());
    if (ptr){
    ...
    ptr = getParentFile()->findHkxObject(eventToSend2.payload.getReference());
    if (ptr){
        if ((*ptr)->getSignature() != HKB_STRING_EVENT_PAYLOAD){
            writeToLog(getClassname()+": linkVar()!\nThe linked object 'payload' is not a HKB_STRING_EVENT_PAYLOAD!");
            setDataValidity(false);
    ...
    ptr = getParentFile()->findHkxObject(eventToSend3.payload.getReference());
    if (ptr){
        if ((*ptr)->getSignature() != HKB_STRING_EVENT_PAYLOAD){
            writeToLog(getClassname()+": linkVar()!\nThe linked object 'payload' is not a HKB_STRING_EVENT_PAYLOAD!");
            setDataValidity(false);
