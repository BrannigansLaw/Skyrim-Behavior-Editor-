...
hkbFootIkControlsModifier::hkbFootIkControlsModifier(BehaviorFile *parent, long ref)
...
bool hkbFootIkControlsModifier::write(HkxXMLWriter *writer){
    ...
    if (!getIsWritten()){
        ...
        writer->writeLine(writer->object, true);
        writer->writeLine(writer->parameter, QStringList(writer->name), QStringList("onOffGain"), QString::number(gains.onOffGain));
        writer->writeLine(writer->parameter, QStringList(writer->name), QStringList("groundAscendingGain"), QString::number(gains.groundAscendingGain));
        writer->writeLine(writer->parameter, QStringList(writer->name), QStringList("groundDescendingGain"), QString::number(gains.groundDescendingGain));
        writer->writeLine(writer->parameter, QStringList(writer->name), QStringList("footPlantedGain"), QString::number(gains.footPlantedGain));
        writer->writeLine(writer->parameter, QStringList(writer->name), QStringList("footRaisedGain"), QString::number(gains.footRaisedGain));
        writer->writeLine(writer->parameter, QStringList(writer->name), QStringList("footUnlockGain"), QString::number(gains.footUnlockGain));
        writer->writeLine(writer->parameter, QStringList(writer->name), QStringList("worldFromModelFeedbackGain"), QString::number(gains.worldFromModelFeedbackGain));
        writer->writeLine(writer->parameter, QStringList(writer->name), QStringList("errorUpDownBias"), QString::number(gains.errorUpDownBias));
        writer->writeLine(writer->parameter, QStringList(writer->name), QStringList("alignWorldFromModelGain"), QString::number(gains.alignWorldFromModelGain));
        writer->writeLine(writer->parameter, QStringList(writer->name), QStringList("hipOrientationGain"), QString::number(gains.hipOrientationGain));
        writer->writeLine(writer->parameter, QStringList(writer->name), QStringList("maxKneeAngleDifference"), QString::number(gains.maxKneeAngleDifference));
        writer->writeLine(writer->parameter, QStringList(writer->name), QStringList("ankleOrientationGain"), QString::number(gains.ankleOrientationGain));
        ...
        for (int i = 0; i < legs.size(); i++){
            ...
            writer->writeLine(writer->parameter, QStringList(writer->name), QStringList("verticalError"), QString::number(legs.at(i).verticalError));
...
bool hkbFootIkControlsModifier::link(){
    ...
    for (int i = 0; i < legs.size(); i++){
        ptr = getParentFile()->findHkxObject(legs.at(i).payload.getReference());
        if (ptr){
