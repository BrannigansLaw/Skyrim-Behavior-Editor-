...
hkbRotateCharacterModifier::hkbRotateCharacterModifier(BehaviorFile *parent, long ref)
...
bool hkbRotateCharacterModifier::write(HkxXMLWriter *writer){
    ...
    if (!getIsWritten()){
        ...
        writer->writeLine(writer->parameter, QStringList(writer->name), QStringList("degreesPerSecond"), getDoubleAsString(degreesPerSecond));
        writer->writeLine(writer->parameter, QStringList(writer->name), QStringList("speedMultiplier"), getDoubleAsString(speedMultiplier));
