...
BSEventEveryNEventsModifier::BSEventEveryNEventsModifier(BehaviorFile *parent, long ref)
...
bool BSEventEveryNEventsModifier::link(){
    ...
    HkxObjectExpSharedPtr *ptr = getParentFile()->findHkxObject(eventToCheckFor.payload.getReference());
    if (ptr){
    ...
    ptr = getParentFile()->findHkxObject(eventToSend.payload.getReference());
    if (ptr){
        if ((*ptr)->getSignature() != HKB_STRING_EVENT_PAYLOAD){
            writeToLog(getClassname()+": linkVar()!\nThe linked object 'payload' is not a HKB_STRING_EVENT_PAYLOAD!");
            setDataValidity(false);
