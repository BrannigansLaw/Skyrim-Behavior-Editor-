...
BSIStateManagerModifier::BSIStateManagerModifier(BehaviorFile *parent, long ref)
      ...
      iStateVar(-1)
{
    ...
    refCount++;
    name = "BSIStateManagerModifier"+QString::number(refCount);
}
            ...
            for (int j = 0; j < numtriggers; j++){
                ...
                while (index < reader.getNumElements() && reader.getNthAttributeNameAt(index, 1) != "class"){
                    if (reader.getNthAttributeValueAt(index, 0) == "pStateMachine"){
                        if (!stateData.last().pStateMachine.readReference(index, reader)){
...
bool BSIStateManagerModifier::link(){
    ...
    for (int i = 0; i < stateData.size(); i++){
        ptr = getParentFile()->findGenerator(stateData.at(i).pStateMachine.data()->getReference());
        if (!ptr){
...
bool BSIStateManagerModifier::evaulateDataValidity(){
    bool valid = true;
    for (int i = 0; i < stateData.size(); i++){
