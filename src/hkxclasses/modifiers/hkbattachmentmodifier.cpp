...
hkbAttachmentModifier::hkbAttachmentModifier(BehaviorFile *parent, long ref)
...
bool hkbAttachmentModifier::link(){
    ...
    HkxObjectExpSharedPtr *ptr = getParentFile()->findHkxObject(sendToAttacherOnAttach.payload.getReference());
    if (ptr){
    ...
    ptr = getParentFile()->findHkxObject(sendToAttacheeOnAttach.payload.getReference());
    if (ptr){
        if ((*ptr)->getSignature() != HKB_STRING_EVENT_PAYLOAD){
            writeToLog(getClassname()+": linkVar()!\nThe linked object 'payload' is not a HKB_STRING_EVENT_PAYLOAD!");
            setDataValidity(false);
    ...
    ptr = getParentFile()->findHkxObject(sendToAttacherOnDetach.payload.getReference());
    if (ptr){
        if ((*ptr)->getSignature() != HKB_STRING_EVENT_PAYLOAD){
            writeToLog(getClassname()+": linkVar()!\nThe linked object 'payload' is not a HKB_STRING_EVENT_PAYLOAD!");
            setDataValidity(false);
    ...
    ptr = getParentFile()->findHkxObject(sendToAttacheeOnDetach.payload.getReference());
    if (ptr){
        if ((*ptr)->getSignature() != HKB_STRING_EVENT_PAYLOAD){
            writeToLog(getClassname()+": linkVar()!\nThe linked object 'payload' is not a HKB_STRING_EVENT_PAYLOAD!");
            setDataValidity(false);
