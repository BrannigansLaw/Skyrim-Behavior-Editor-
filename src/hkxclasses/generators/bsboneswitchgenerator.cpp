...
int BSBoneSwitchGenerator::addChildrenToList(QList <HkxObjectExpSharedPtr> & list, bool reverseOrder){
    ...
    if (reverseOrder){
        for (int i = ChildrenA.size() - 1; i >= 0; i--){
            if (ChildrenA.at(i).data() && static_cast<BSBoneSwitchGeneratorBoneData *>(ChildrenA.at(i).data())->pGenerator.data()){
                list.append(static_cast<BSBoneSwitchGeneratorBoneData *>(ChildrenA.at(i).data())->pGenerator);
                objectChildCount++;
            }
        }
    }else{
