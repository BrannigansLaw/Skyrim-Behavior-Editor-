...
QString BSDirectAtModifier::evaluateDataValidity(){
    ...
    if (sourceBoneIndex >= static_cast<BehaviorFile *>(getParentFile())->getNumberOfBones()){
        isvalid = false;
        errors.append(getParentFile()->getFileName()+": "+getClassname()+": Ref: "+getReferenceString()+": "+getName()+": sourceBoneIndex out of range! Setting to last bone index!\n");
        sourceBoneIndex = static_cast<BehaviorFile *>(getParentFile())->getNumberOfBones() - 1;
    }
